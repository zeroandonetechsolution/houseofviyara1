...
new_modal = '''    <div class="modal-overlay" id="auth-overlay"></div>
...
for path in sorted(html_files):
    ...
    if '<div class="auth-modal" id="auth-modal">' not in text:
        continue
    start = text.index('<div class="modal-overlay" id="auth-overlay"></div>')
    if start == -1:
        continue
    end = text.find('<div class="mobile-menu"', start)
    if end == -1:
        end = text.find('<div class="cart-drawer-overlay"', start)
    if end == -1:
