...
async function supabaseUploadFile(fileOrData, pathPrefix = 'products') {
  ...
  // --- Use client-side conversion for HEIC/HEIF! ---
  if (isHeicFile(file)) {
...
async function renderInventory() {
    ...
    try {
            ...
            <table class="admin-table">
              ...
              <tbody>
                ${products.map(p => {
                  ...
                  const stockStatus = stock === 0 ? 'out-of-stock' : isLowStock ? 'low-stock' : 'in-stock';
                  return `
                    <tr>
                      <td>
                        <div style="display:flex;align-items:center;gap:10px;">
                          <img src="${p.image_url}" alt="${p.name}" style="width:50px;height:50px;object-fit:cover;border-radius:4px;" onerror="this.src='https://via.placeholder.com/50?text=No+Image'">
                          <strong>${p.name}</strong>
                        </div>
                      </td>
                      <td>${p.category}</td>
                      <td>
                        <input type="number" class="aform-input" id="stock-${p.id}" value="${stock}" style="width:100px;">
                      </td>
                      <td>
                        <input type="number" class="aform-input" id="threshold-${p.id}" value="${threshold}" style="width:100px;">
                      </td>
                      <td>
        ...
        content.innerHTML = `<div class="admin-error"><i class="fas fa-exclamation-triangle"></i><p>Failed to load inventory.</p><code>${e.message}</code></div>`;
    }
}
    ...
    <div class="admin-form">
      ...
      <div class="aform-row">
        ...
        <div class="aform-group">
          ...
          <input class="aform-input" id="pf-low-stock-threshold" type="number" value="${p.low_stock_threshold || 5}" placeholder="5">
        </div>
      </div>
